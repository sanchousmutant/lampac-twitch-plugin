    ...
    public class Startup
    {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, IMemoryCache memory, IHttpClientFactory httpClientFactory, IHostApplicationLifetime applicationLifetime)
        {
            ...
            app.UseEndpoints(endpoints =>
            {
                ...
                endpoints.MapPost("/twitch/proxy", async context =>
                {
                    try
                    {
                ...
                endpoints.MapGet("/twitch/stream", async context =>
                {
                    try
                    {
                        ...
                        if (string.IsNullOrWhiteSpace(channel))
                        {
                            ...
                            await context.Response.WriteAsJsonAsync(new { error = "channel is required" });
                            return;
                        }

                        var requestInfo = context.Features.Get<Shared.Models.RequestModel>();
                        string reqip = requestInfo?.IP;

                        var client = httpClientFactory.CreateClient();
                        client.Timeout = TimeSpan.FromSeconds(10);

                        // PlaybackAccessToken via persisted GQL query (hash from streamlink 8.2.1)
                        var gqlBody = System.Text.Json.JsonSerializer.Serialize(new
                        {
                        ...
                        var gqlRequest = new HttpRequestMessage(HttpMethod.Post, "https://gql.twitch.tv/gql");
                        gqlRequest.Headers.Add("Client-ID", "kimne78kx3ncx6brgo4mv6wki5h1ko");
                        gqlRequest.Content = new StringContent(gqlBody, System.Text.Encoding.UTF8, "application/json");

                        var gqlResponse = await client.SendAsync(gqlRequest);
                        var gqlContent = await gqlResponse.Content.ReadAsStringAsync();

                        using var doc = System.Text.Json.JsonDocument.Parse(gqlContent);
                        var data = doc.RootElement.GetProperty("data");
                        var token = data.GetProperty("streamPlaybackAccessToken");

                        string tokenValue = token.GetProperty("value").GetString();
                        string tokenSignature = token.GetProperty("signature").GetString();

                        // Build Usher v2 HLS URL
                        var rnd = new Random();
                        string usherUrl = $"https://usher.ttvnw.net/api/v2/channel/hls/{Uri.EscapeDataString(channel.ToLower())}.m3u8"
                            + $"?token={Uri.EscapeDataString(tokenValue)}"
                            + $"&sig={Uri.EscapeDataString(tokenSignature)}"
                            + "&allow_source=true&allow_audio_only=true"
                            + "&supported_codecs=h264"
                            + "&platform=web"
                            + $"&p={rnd.Next(0, 999999)}";

                        // Encrypt URL for proxy passthrough
                        string hash = ProxyLink.Encrypt(usherUrl, reqip, verifyip: false, ex: DateTime.Now.AddHours(6));

                        string host = $"{context.Request.Scheme}://{context.Request.Host.Value}";

                        context.Response.ContentType = "application/json";
                        await context.Response.WriteAsJsonAsync(new { url = $"{host}/proxy/{hash}" });
                    }
                    catch (Exception ex)
                    {
                        context.Response.StatusCode = 500;
                ...
                endpoints.MapGet("/twitch/vod", async context =>
                {
                    try
                    {
                        string vodId = context.Request.Query["id"];
                        if (string.IsNullOrWhiteSpace(vodId))
                        {
                            context.Response.StatusCode = 400;
                            await context.Response.WriteAsJsonAsync(new { error = "id is required" });
                            return;
                        }
                        ...
                        using var doc = System.Text.Json.JsonDocument.Parse(gqlContent);
                        var data = doc.RootElement.GetProperty("data");
                        var token = data.GetProperty("videoPlaybackAccessToken");

                        string tokenValue = token.GetProperty("value").GetString();
                        string tokenSignature = token.GetProperty("signature").GetString();

                        var rnd = new Random();
                        string usherUrl = $"https://usher.ttvnw.net/vod/v2/{Uri.EscapeDataString(vodId)}.m3u8"
                            + $"?nauth={Uri.EscapeDataString(tokenValue)}"
                            + $"&nauthsig={Uri.EscapeDataString(tokenSignature)}"
                            + "&allow_source=true&allow_audio_only=true"
                            + "&supported_codecs=h264"
                            + "&platform=web"
                            + $"&p={rnd.Next(0, 999999)}";

                        string hash = ProxyLink.Encrypt(usherUrl, reqip, verifyip: false, ex: DateTime.Now.AddHours(12));
